...
def get_projection_matrix(
    intrinsics, extrinsics, return_numpy=False, downscale: float = 1.0
) -> Union[Tensor, np.ndarray]:
    """
    Computes the projection matrix given the intrinsic and extrinsic camera parameters.

    Args:
        intrinsics (np.ndarray or torch.Tensor): The intrinsic camera parameters.
        extrinsics (np.ndarray or torch.Tensor): The extrinsic camera parameters.
        return_numpy (bool): Whether to return the projection matrix as a numpy array.
        downscale (float): The amount to downscale the intrinsics by.

    Returns:
        torch.Tensor or np.ndarray: The projection matrix.
    """
    if isinstance(intrinsics, np.ndarray):
        intrinsics = torch.from_numpy(intrinsics).float()
    if isinstance(extrinsics, np.ndarray):
        extrinsics = torch.from_numpy(extrinsics).float()
    projection = torch.eye(4, dtype=torch.float32, device=extrinsics.device)
    projection[:3, :3] = intrinsics / downscale
    projection[2, 2] = 1.0
    projection = projection @ torch.linalg.inv(extrinsics)
    if return_numpy:
...
def point_sampling(
    ...
    """
    Samples points from a 3D world coordinate tensor and projects them onto a 2D image plane.

    Args:
        world_coords (torch.Tensor): A tensor of shape (B, num_queries, 3) containing the 3D world coordinates of the points to be sampled.
        proj_mat (torch.Tensor): A tensor of shape (B, num_cams, 4, 4) containing the projection matrices for each camera.
        img_height (int): The height of the image plane in pixels. Default is 320.
        img_width (int): The width of the image plane in pixels. Default is 480.

    Returns:
        A tuple containing three tensors:
        - reference_points_cam (torch.Tensor): A tensor of shape (B, num_cams, num_queries, 2) containing the 2D image coordinates of the sampled points.
        - mask (torch.Tensor): A tensor of shape (B, num_cams, num_queries) containing a binary mask indicating which points are visible in each camera.
        - depth (torch.Tensor): A tensor of shape (B, num_cams, num_queries, 1) containing the depth of each sampled point in each camera.
    """
    B, num_cams = proj_mat.shape[:2]
    num_queries = world_coords.shape[1]
    reference_points = torch.cat(
        (
            world_coords,
            torch.ones_like(world_coords[..., :1], device=world_coords.device),
        ),
        -1,
    ).unsqueeze(1)
    proj_mat = proj_mat.to(world_coords.device)
    reference_points_cam = (proj_mat @ reference_points.transpose(-1, -2)).transpose(
        -1, -2
    )
    depth = reference_points_cam[..., 2:3]
    reference_points_cam = reference_points_cam[..., :2] / (
        reference_points_cam[..., 2:3] + 1e-6
    )  # normalize by z
    reference_points_cam[..., 0] = reference_points_cam[..., 0] / img_width * 2 - 1
    reference_points_cam[..., 1] = reference_points_cam[..., 1] / img_height * 2 - 1
    mask = (
    ...
    mask = mask.view(B, num_cams, num_queries)
    depth = depth.view(B, num_cams, num_queries)
    reference_points_cam = reference_points_cam.view(B, num_cams, -1, 2)
    return reference_points_cam, mask, depth
...
def get_ground_np(pts):
    ...
    """
    # from https://github.com/tusen-ai/LiDAR_SOT/blob/main/waymo_data/data_preprocessing/ground_removal.py
    th_seeds_ = 1.2
    num_lpr_ = 20
    n_iter = 10
    th_dist_ = 0.3
