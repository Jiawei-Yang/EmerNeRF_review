...
class RealValueLoss(Loss):
    ...
    def __call__(
        ...
        """
        Compute the loss between the estimated and ground truth values.

        Args:
            estimate (Tensor): The estimated values.
        ...
        Returns:
            The loss value.
        """
        gt, estimate = gt.squeeze(), estimate.squeeze()
        loss = self.loss_fn(estimate, gt, reduction="none")
        if mask is not None:
...
class DepthLoss(Loss):
    ...
    Args:
        ...
        depth_percentile (float): Percentile of depth values to use.
    ...
    Attributes:
        ...
        name (str): Name of the loss.
        upper_bound (float): Truncation value for the depth values.
        depth_percentile (float): Percentile of depth values being used.
    """
    ...
    def __call__(
        ...
        if self.depth_percentile is not None:
            depth_error = depth_error.flatten()
            depth_error = depth_error[
                depth_error.argsort()[: int(len(depth_error) * self.depth_percentile)]
            ]
...
class LineOfSightLoss(Loss):
    ...
    Args:
        ...
        name (str): The name of the loss function.
        depth_percentile (float): The percentile of rays to optimize within each batch that have smallest depth error.
        coef (float): The coefficient to multiply the loss by.
