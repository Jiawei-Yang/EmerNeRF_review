...
def extract_and_save_dino_features(
    image_list: List[str],
    ...
    if "clip" in model_type:
        dino_model_type = "clip"
    else:
        dino_model_type = "dinov2" if "dinov2" in model_type else "dino"
    for i in trange(
        0, len(image_list), desc="Extracting DINO features (will skip existing files)"
    ):
        image_pth = image_list[i]
        if i == 0:
            os.makedirs(
                os.path.dirname(
                    image_pth.replace(
                        "training", f"training_{dino_model_type}"
                    ).replace("images", "dino")
                ),
                exist_ok=True,
            )
        dino_pth = (
            image_pth.replace("training", f"training_{dino_model_type}")
            .replace("images", "dino")
            .replace("jpg", "npy")
        )

        file_exists = os.path.exists(dino_pth)
        if file_exists and not return_pca:
            continue
        ...
        if return_pca and i < num_cams:
            reduction_mat, color_min, color_max = get_robust_pca(
                descriptors.reshape(-1, descriptors.shape[-1])
            )
            X = descriptors @ reduction_mat
            X = (X - color_min) / (color_max - color_min)
            X = (
                F.interpolate(
                    X.permute(2, 0, 1).unsqueeze(0), size=img_shape, mode="nearest"
                )
                .squeeze()
                .permute(1, 2, 0)
            )
            X = to8b(X.cpu().detach().numpy())
            img = Image.open(image_list[i]).convert("RGB").resize(img_shape[::-1])
            # concatenate the image and the PCA_maps
            img = np.concatenate((np.array(img), X), axis=0)
            pca_imgs.append(img)
        descriptors = descriptors.cpu().detach().numpy()
        if not file_exists:
            np.save(dino_pth, descriptors)
    del extractor
    # clear the cache
    torch.cuda.empty_cache()
    if return_pca:
        # change the order of the images
        pca_imgs = np.array(pca_imgs)
        if num_cams == 5:
            return np.concatenate(pca_imgs[[3, 1, 0, 2, 4]], axis=1)
        elif num_cams == 6:
            return np.concatenate(pca_imgs[[0, 1, 2, 3, 4, 5]], axis=1)
        elif num_cams == 3:
            return np.concatenate(pca_imgs[[1, 0, 2]], axis=1)
    else:
        return None
