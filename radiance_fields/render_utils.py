...
def rendering(
    ...
    Returns:
        Dict[str, Tensor]: A dictionary of rendered information about the scene, including density, depth, opacity, and rgb values.
    """
    ...
    if "rgb" in results:
        ...
        # =========== RGB Decomposition =========== #
        if return_decomposition:
            static_rgb = accumulate_along_rays(
                static_weights,
                values=results["static_rgb"],
            )
            results_dict["static_rgb"] = static_rgb
            if "shadow_ratio" in results:
...
def render_rays(
    ...
    proposal_networks: Optional[Sequence[DensityField]] = None,
    ...
    for sup_key in ["depth", "sky_mask", "dino_feat"]:
        # these keys are for supervision. remove them from the input data_dict
        if sup_key in reshaped_data_dict:
            reshaped_data_dict.pop(sup_key)

    def prop_sigma_fn(t_starts, t_ends, proposal_network):
        # query propsal networks for sigma
    ...
    def query_fn(t_starts, t_ends):
        ...
        sub_dict = {
            k: v[..., None].repeat_interleave(t_starts.shape[-1], dim=-1)
            for k, v in chunk_data_dict.items()
            if k not in [prefix + "viewdirs", prefix + "origins", "pix_coords"]
        }
        sub_dict["t_starts"] = t_starts
        sub_dict["t_ends"] = t_ends
        if "pix_coords" in chunk_data_dict:
            # use this for DINO positional embedding decomposition
            sub_dict["pix_coords"] = chunk_data_dict["pix_coords"]
        positions = t_origins + t_dirs * (t_starts + t_ends)[..., None] / 2.0
        ...
        # try to query sky
        if prefix != "lidar_":
            # there will be no sky for lidar rays
            results_dict.update(
                radiance_field.query_sky(
                    chunk_data_dict["viewdirs"],
                    {
                        k: v
                        for k, v in chunk_data_dict.items()
                        if k in ["cam_idx", "img_idx"]
                    },
                )
            )
        return results_dict
    ...
    for i in range(0, num_rays, chunk):
        ...
        t_starts, t_ends = prop_estimator.sampling(
            prop_sigma_fns=[
                lambda *args: prop_sigma_fn(*args, p) for p in proposal_networks
            ],
            num_samples=cfg.nerf.sampling.num_samples,
            prop_samples=cfg.nerf.estimator.propnet.num_samples_per_prop,
            n_rays=chunk_data_dict[prefix + "origins"].shape[0],
            near_plane=cfg.nerf.estimator.propnet.near_plane,
            far_plane=cfg.nerf.estimator.propnet.far_plane,
            sampling_type=cfg.nerf.estimator.propnet.sampling_type,
            stratified=radiance_field.training,
            requires_grad=proposal_requires_grad,
            device=chunk_data_dict[prefix + "origins"].device,
        )
        # render the scene
